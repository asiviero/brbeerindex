...
input_file = 'winedb.csv'
output_file = 'csv_example_output_wine.csv'
settings_file = 'csv_example_learned_settings_wine'
training_file = 'csv_example_training_wine.json'
    ...
    with open(filename) as f:
        ...
        for i, row in enumerate(reader):
            ...
            try :
                ...
                clean_row['price'] = 0
            # try:
            #     clean_row["name"] = re.sub("cerveja","",clean_row["name"])
            # except:
            #     clean_row["name"] = ""
            #     pass
...
if os.path.exists(settings_file):
    ...
    # for the 'Zip' field.
    fields = [
        {'field' : 'grape', 'type':'String','has missing':True},
        {'field' : 'alcohol', 'type':'String','has missing':True},
        {'field' : 'country', 'type':'String','has missing':True},
        {'field' : 'name', 'type': 'String'},
        {'field' : 'volume', 'type': 'String'},
        {'field' : 'winery', 'type': 'String','has missing':True},
        {'field' : 'winetype', 'type':'String','has missing':True},
        {'field' : 'region', 'type':'String','has missing':True},
...
with open(output_file, 'w') as f_output:
    ...
    with open(input_file) as f_input :
        ...
        for row_id,row in enumerate(reader):
            key = input_file+str(row_id)
            #row_id = int(row[0])
            if (key) in cluster_membership :
                cluster_id = cluster_membership[key]["cluster id"]
                #canonical_rep = cluster_membership[row_id]["canonical representation"]
                row.insert(0, cluster_membership[key]['confidence'])
